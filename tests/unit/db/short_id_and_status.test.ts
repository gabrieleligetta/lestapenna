...
describe('Universal Short ID & Quest Status Tests', () => {
    ...
    test('Short ID lookup tollera maiuscole, prefisso # e spazi', () => {
        const npcName = 'Test NPC SID Case';
        updateNpcEntry(campaignId, npcName, 'Desc', 'Role', 'ALIVE', undefined, true);
        const dbInstance = require('../../../src/db').db;
        const rec = dbInstance.prepare('SELECT short_id FROM npc_dossier WHERE name = ? AND campaign_id = ?').get(npcName, campaignId);
        expect(rec.short_id).toBeDefined();

        expect(getNpcByShortId(campaignId, rec.short_id.toUpperCase())?.name).toBe(npcName);
        expect(getNpcByShortId(campaignId, `#${rec.short_id}`)?.name).toBe(npcName);
        expect(getNpcByShortId(campaignId, ` ${rec.short_id.toUpperCase()} `)?.name).toBe(npcName);
    });
    ...
    test('Repository Lookup by Short ID', () => {
        ...
        upsertMonster(campaignId, monsterName, 'ALIVE', 'session-1', undefined, undefined, true);
