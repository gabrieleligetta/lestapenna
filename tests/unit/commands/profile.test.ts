...
// Enrolling at the table touches the real DB: what matters here is only that `$iam`
// calls it. The behaviour is covered by tests/unit/db/campaign_membership.test.ts.
jest.mock('../../../src/services/campaignAccess', () => ({
    ensureMembership: jest.fn(),
    canWriteCampaign: jest.fn().mockReturnValue(true),
    getCampaignRole: jest.fn().mockReturnValue('PLAYER'),
