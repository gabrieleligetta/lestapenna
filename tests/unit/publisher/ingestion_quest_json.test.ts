...
// SessionPhaseManager uses the real SQLite client: without a mock it would require the
// schema to be initialized (it used to pass only thanks to the leftover schema of the shared DB).
jest.mock('../../../src/services/SessionPhaseManager', () => ({
    sessionPhaseManager: { setPhase: jest.fn(), getPhase: jest.fn(() => null), markFailed: jest.fn() }
}));
jest.mock('../../../src/db', () => ({
    addQuest: jest.fn(),
