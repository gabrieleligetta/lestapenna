...
describe('CommandDispatcher Integration', () => {
    ...
    beforeEach(() => {
        ...
        (getGuildConfig as jest.Mock).mockReturnValue('channel-123'); // Default: channel configured
    ...
    it('should block command requiring campaign if none active', async () => {
        ...
        expect(testCommand.execute).not.toHaveBeenCalled();
        // Language-agnostic assertion: the blocking message always cites $creacampagna,
        // whichever language the guild resolves to (the mock has no preferredLocale → en).
        expect(replyMock).toHaveBeenCalledWith(expect.stringContaining('creacampagna'));
    });
});
