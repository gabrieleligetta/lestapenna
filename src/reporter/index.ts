...
import { buildSessionDiagnosticsAttachment } from '../services/audioDiagnostics';

export { processSessionReport } from './generator';
export { archiveSessionTranscripts } from './archives';
export { sendTestEmail } from './testing';
...
export async function sendSessionRecap(
    ...
    // 1. Generation and archiving (always, even when email is disabled)
    ...
    try {
        ...
        // 🆕 Report diagnostico audio (sync/rumore per traccia) da allegare in JSON
        let audioDiagPath: string | null = null;
        try {
            audioDiagPath = buildSessionDiagnosticsAttachment(sessionId);
        } catch (e) {
            console.warn(`[Reporter] ⚠️ Diagnostica audio non disponibile per ${sessionId}:`, e);
        }
...
<div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; color: #333;">
    ...
    <ul style="color: #7f8c8d;">
        ...
        ${audioDiagPath ? `<li><strong>Diagnostica Audio (JSON):</strong> metriche di sincronizzazione per ogni traccia — drift, silenzio iniettato, gap di rotazione, delay nel mix e durata reale vs attesa</li>` : ''}
    </ul>
    ...
    <p style="font-size: 11px; color: #95a5a6; text-align: center;">Generato automaticamente dal Bardo AI Lestapenna.</p>
        ...
        if (audioDiagPath && fs.existsSync(audioDiagPath)) attachments.push({ path: audioDiagPath });
        ...
        // Cleanup (the aggregate JSON can be regenerated from the source JSONL, which stays as archive)
        if (filePaths) {
            try {
                if (fs.existsSync(filePaths.cleaned)) fs.unlinkSync(filePaths.cleaned);
                if (fs.existsSync(filePaths.raw)) fs.unlinkSync(filePaths.raw);
        ...
        try {
            if (audioDiagPath && fs.existsSync(audioDiagPath)) fs.unlinkSync(audioDiagPath);
        } catch (e) { }
