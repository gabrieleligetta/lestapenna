...
describe('Normalization Logic', () => {
    ...
    beforeEach(() => {
        ...
        (reconcileNpcName as jest.MockedFunction<typeof reconcileNpcName>).mockResolvedValue(null);
    });
    ...
    it('should maintain canonical name if reconciliation finds one', async () => {
        ...
        (reconcileNpcName as jest.MockedFunction<typeof reconcileNpcName>).mockResolvedValue({ canonicalName: 'Canonical Pari', existingNpc: {} });
    ...
    it('should remove player characters from NPC collections', async () => {
        (reconcileNpcName as jest.MockedFunction<typeof reconcileNpcName>).mockImplementation(async (_campaignId: number, name: string) => {
            if (name === 'Piero') {
                return { canonicalName: 'Piero', existingNpc: null, isPlayerCharacter: true };
            }
            return null;
        });

        const input = {
            present_npcs: ['Piero', 'Pier'],
            npc_dossier_updates: [
                { name: 'Piero', description: 'Barbaro del party' },
                { name: 'Pier', description: 'Paladino di Tyr' }
            ],
            npc_events: [
                { name: 'Piero', type: 'STATUS_CHANGE', event: 'Scompare' },
                { name: 'Pier', type: 'REVELATION', event: 'Predica' }
            ]
        };

        const result = await normalizeSummaryNames(CAMPAIGN_ID, input);

        expect(result.present_npcs).toEqual(['Pier']);
        expect(result.npc_dossier_updates.map((u: any) => u.name)).toEqual(['Pier']);
        expect(result.npc_events.map((e: any) => e.name)).toEqual(['Pier']);
    });
});
