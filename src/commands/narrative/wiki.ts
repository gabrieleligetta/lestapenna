...
export const wikiCommand: Command = {
    ...
    async execute(ctx: CommandContext): Promise<void> {
        ...
        if (!term) {
            await message.reply(t(ctx.locale, 'narrative.wikiUsage'));
            return;
        }
        ...
        try {
            ...
            if (fragments.length === 0) {
                await message.reply(t(ctx.locale, 'narrative.wikiNothing'));
                return;
            }
            ...
            const buildEmbed = (i: number) => {
                ...
                const content = fragments[i].length > MAX_LENGTH
                    ? fragments[i].substring(0, MAX_LENGTH) + t(ctx.locale, 'narrative.wikiTruncated')
                    : fragments[i];
                return new EmbedBuilder()
                    .setTitle(t(ctx.locale, 'narrative.wikiArchiveTitle', { term }))
                    .setDescription(content)
                    .setColor(0x8B4513)
                    .setFooter({ text: t(ctx.locale, 'narrative.wikiFragment', { i: i + 1, total }) });
            };
            ...
            await message.reply(t(ctx.locale, 'narrative.wikiError'));
        }
    }
};
