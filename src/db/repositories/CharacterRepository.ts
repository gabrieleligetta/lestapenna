...
import { UserProfile, CharacterHistoryEntry, AffiliationRole } from '../types';
import { computeAlignmentFromHistory } from './shared';
...
export const characterRepository = {
    addCharacterEvent: (campaignId: number, charName: string, sessionId: string, description: string, type: string, isManual: boolean = false, timestamp?: number, moral_weight: number = 0, ethical_weight: number = 0, factionId?: number) => {
        ...
        // 2. Update Character Score: shared aggregation formula
        // (repositories/shared.ts), identical to NPCs/Factions.
        const { moralScore, ethicalScore, moralLabel, ethicalLabel } =
            computeAlignmentFromHistory(campaignId, 'character_history', 'character_name', charName);
        db.prepare(`
            UPDATE characters
            SET moral_score = ?,
                ethical_score = ?,
        ...
        `).run(moralScore, ethicalScore, moralLabel, ethicalLabel, campaignId, charName);

        // Party alignment is driven by faction_history events only (via addFactionEvent).
        // Character events affect only the character's own score.
    ...
    // Alignment columns are selected here too (additive: callers that ignore them are
    // unaffected) so the roster can show alignment without an N+1 of getUserProfile.
    // `email` stays out on purpose — it is per-owner PII, see the API's character detail.
    getCampaignCharacters: (campaignId: number): (UserProfile & { user_id: string, foundation_description: string | null })[] => {
        return db.prepare('SELECT user_id, character_name, race, class, description, foundation_description, alignment_moral, alignment_ethical, moral_score, ethical_score FROM characters WHERE campaign_id = ?').all(campaignId) as any[];
    },

    /**
     * The party roster: every character in the campaign, plus the role its
     * faction affiliation gives it, if any.
     *
     * Built from characters LEFT JOIN affiliations, not from the affiliations
     * alone: ensurePartyMembership never affiliates the DM, so an
     * affiliation-driven roster silently drops them.
     *
     * `email` stays out — it is per-owner PII, see the API's character detail.
     */
    getPartyRoster: (campaignId: number): Array<UserProfile & {
        user_id: string;
        role: AffiliationRole | null;
    }> => {
        return db.prepare(`
            SELECT c.user_id, c.character_name, c.race, c.class, c.description,
                   c.alignment_moral, c.alignment_ethical, c.moral_score, c.ethical_score,
                   fa.role as role
            FROM characters c
            LEFT JOIN factions f ON f.campaign_id = c.campaign_id AND f.is_party = 1
            LEFT JOIN faction_affiliations fa
                   ON fa.faction_id = f.id AND fa.entity_type = 'pc'
                  AND fa.entity_id = c.rowid AND fa.is_active = 1
            WHERE c.campaign_id = ?
            ORDER BY c.character_name
        `).all(campaignId) as any[];
    },

    /** The rowid faction_affiliations addresses this character by. */
    getCharacterRowId: (userId: string, campaignId: number): number | null => {
        const row = db.prepare('SELECT rowid FROM characters WHERE user_id = ? AND campaign_id = ?')
    ...
    updateUserCharacter: (userId: string, campaignId: number, field: 'character_name' | 'race' | 'class' | 'description' | 'foundation_description' | 'email', value: string, isManual: boolean = true): void => {
        ...
        // An email change needs no RAG sync
