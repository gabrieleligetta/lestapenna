...
import { getByShortId, computeAlignmentFromHistory, resolveEntityId, getHistoryByEntity, listEntities, countEntities, ListOptions } from './shared';
import { NpcEntry } from '../types';
import { generateShortId } from '../utils/idGenerator';
import { getMoralAlignment, getEthicalAlignment, computeAggregatedAlignmentScore } from '../../utils/alignmentUtils';
...
export const npcRepository = {
    updateNpcEntry: (campaignId: number, name: string, description: string, role?: string, status?: string, sessionId?: string, isManual: boolean = false, moral?: string, ethical?: string) => {
        ...
        // Upsert - IMPORTANT: last_updated_session_id tracks who modified it last (for a clean purge)
        db.prepare(`
    ...
    // `opts` is additive: existing call sites pass limit/offset positionally and
    // keep the previous behaviour exactly.
    listNpcs: (campaignId: number, limit: number = 10, offset: number = 0, opts: ListOptions = {}): NpcEntry[] =>
        listEntities<NpcEntry>('npc_dossier', '*', campaignId, 'last_updated DESC', { limit, offset, ...opts }),

    countNpcs: (campaignId: number, opts: ListOptions = {}): number =>
        countEntities('npc_dossier', campaignId, opts),
    ...
    addNpcEvent: (campaignId: number, npcName: string, sessionId: string, description: string, type: string, isManual: boolean = false, timestamp?: number, moral_weight: number = 0, ethical_weight: number = 0, factionId?: number) => {
        ...
        db.transaction(() => {
            // 1. Insert Event (entity_id: a stable link to the NPC, it survives renames)
            const entityId = resolveEntityId('npc_dossier', 'name', campaignId, npcName);
            db.prepare(`
                INSERT INTO npc_history (campaign_id, npc_name, session_id, description, event_type, timestamp, is_manual, moral_weight, ethical_weight, faction_id, entity_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).run(campaignId, npcName, sessionId, description, type, timestamp || Date.now(), isManual ? 1 : 0, moral_weight, ethical_weight, factionId || null, entityId);

            // 2. Update Scores: shared aggregation formula (repositories/shared.ts),
            // identical to PCs/Factions.
            const { moralScore, ethicalScore, moralLabel, ethicalLabel } =
                computeAlignmentFromHistory(campaignId, 'npc_history', 'npc_name', npcName, entityId);
            db.prepare(`
                UPDATE npc_dossier
                SET moral_score = ?,
                    ethical_score = ?,
            ...
            `).run(moralScore, ethicalScore, moralLabel, ethicalLabel, campaignId, npcName);

            // 3. Handle Faction Link (Member vs Interaction)
    ...
    getNpcHistory: (campaignId: number, npcName: string): any[] => {
        // id-first (segue rename/merge) + fallback nome; ora case-insensitive
        return getHistoryByEntity('npc_history', 'npc_name', 'npc_dossier', 'name', campaignId, npcName);
    },
    ...
    renameNpcEntry: (campaignId: number, oldName: string, newName: string): boolean => {
        ...
        db.transaction(() => {
            if (conflict) {
                ...
                // Repoint the history to the target (name + entity_id, before deleting
                // the source dossier: rows with the old id must not remain)
                db.prepare(`
                    UPDATE npc_history
                    SET npc_name = ?, entity_id = ?
                    WHERE campaign_id = ?
                      AND (entity_id = ? OR (entity_id IS NULL AND lower(npc_name) = lower(?)))
                `).run(newName, conflict.id, campaignId, existing.id, oldName);
                ...
                // RENAME SEMPLICE (la history segue nome + entity_id)
                ...
                db.prepare(`
                    ...
                    WHERE campaign_id = ?
                      AND (entity_id = ? OR (entity_id IS NULL AND lower(npc_name) = lower(?)))
                `).run(newName, existing.id, campaignId, existing.id, oldName);
    ...
    /**
     * Finds the NPCs named in `query` (one question, or several concatenated).
     * NB: the match is "query CONTAINS name/alias", not the other way round — a
     * short NPC name ("Helena") can never contain a whole question as a
     * substring, so the direction matters.
     */
    findNpcDossierByName: (campaignId: number, query: string): any[] => {
        const queryLower = query.toLowerCase();
        const allNpcs = db.prepare(`
            SELECT short_id, name, role, status, description, aliases
            FROM npc_dossier
            WHERE campaign_id = ?
        `).all(campaignId) as Array<{ short_id: string; name: string; role: string; status: string; description: string; aliases: string | null }>;

        const matches = allNpcs.filter(npc => {
            const nameLower = (npc.name || '').toLowerCase();
            if (nameLower && queryLower.includes(nameLower)) return true;
            if (npc.aliases) {
                const aliases = npc.aliases.split(',').map(a => a.trim().toLowerCase()).filter(Boolean);
                if (aliases.some(alias => queryLower.includes(alias))) return true;
            }
            return false;
        });
    ...
    getNpcByShortId: (campaignId: number, shortId: string): NpcEntry | null =>
        getByShortId<NpcEntry>('npc_dossier', campaignId, shortId),
