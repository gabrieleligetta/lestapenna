...
export type EntityType = 'npc' | 'location' | 'faction' | 'artifact' | 'item' | 'quest' | 'monster';
...
export interface EntityIndex {
    ...
    items: Map<string, IndexedEntity>;
    quests: Map<string, IndexedEntity>;
    monsters: Map<string, IndexedEntity>;
...
export function buildEntityIndex(campaignId: number): EntityIndex {
    ...
    // 6. Index Items (inventory - deduplicated by name)
    try {
        const items = listAllInventory(campaignId);
        for (const item of items) {
            const normalized = normalizeForIndex(item.item_name);
            if (index.items.has(normalized)) continue; // Keep first occurrence only
            const trigrams = extractTrigrams(item.item_name);
            ...
            index.items.set(normalized, entity);
            addToTrigramIndex(index.trigramIndex, trigrams, `item:${normalized}`);
        }
    } catch (e) {
        // Inventory might not exist for this campaign
    }

    // 7. Index Quests
    try {
        const quests = listAllQuests(campaignId);
        for (const quest of quests) {
            const normalized = normalizeForIndex(quest.title);
            const trigrams = extractTrigrams(quest.title);
            ...
            index.quests.set(normalized, entity);
            addToTrigramIndex(index.trigramIndex, trigrams, `quest:${normalized}`);
        }
    } catch (e) {
        // Quests might not exist for this campaign
    }

    // 8. Index Monsters (bestiary)
    try {
        const monsters = listAllMonsters(campaignId);
        for (const monster of monsters) {
            const normalized = normalizeForIndex(monster.name);
            const trigrams = extractTrigrams(monster.name);
            ...
            index.monsters.set(normalized, entity);
            addToTrigramIndex(index.trigramIndex, trigrams, `monster:${normalized}`);
        }
    } catch (e) {
        // Monsters might not exist for this campaign
    }

    const elapsed = Date.now() - startTime;
    console.log(`[EntityIndex] ✅ Index built in ${elapsed}ms: ${index.npcs.size} NPCs, ${index.locations.size} locations, ${index.factions.size} factions, ${index.artifacts.size} artifacts, ${index.items.size} items, ${index.quests.size} quests, ${index.monsters.size} monsters, ${index.playerCharacters.size} PCs`);
...
function getEntityMap(index: EntityIndex, type: EntityType): Map<string, IndexedEntity> {
    switch (type) {
        case 'npc': return index.npcs;
        case 'location': return index.locations;
        case 'faction': return index.factions;
        case 'artifact': return index.artifacts;
        case 'item': return index.items;
        case 'quest': return index.quests;
        case 'monster': return index.monsters;
    }
}

export function getAllEntitiesOfType(index: EntityIndex, type: EntityType): IndexedEntity[] {
    return Array.from(getEntityMap(index, type).values());
}
...
export function localMatch(
    ...
    name: string,
    type: EntityType,
    context?: { currentMacro?: string; currentMicro?: string; description?: string }
): MatchCandidate[] {
    const normalized = normalizeForIndex(name);
    const queryTrigrams = extractTrigrams(name);
    const candidates: MatchCandidate[] = [];

    const entityMap = getEntityMap(index, type);
    ...
    const prefixMap: Record<EntityType, string> = {
        npc: 'npc:', location: 'loc:', faction: 'faction:',
        artifact: 'artifact:', item: 'item:', quest: 'quest:', monster: 'monster:'
    };
    ...
    // 6. Description cross-check (alias detection, zero API cost)
    // Only when no good string-match candidates found AND a description is provided.
    // Checks if the query name appears in an existing entity's description (e.g. "nota come La Strega"),
    // or if an existing entity's name appears in the new entity's description.
    const hasGoodCandidate = candidates.some(c => c.score >= 0.5);
    if (!hasGoodCandidate && context?.description && context.description.length > 15) {
        ...
        for (const [, entity] of entityMap) {
            ...
            if (entity.description && entity.description.toLowerCase().includes(nameLower)) {
                candidates.push({ entity, score: 0.87, reason: 'desc_mentions_query' });
            } else if (descLower.includes(entity.name.toLowerCase())) {
                candidates.push({ entity, score: 0.84, reason: 'query_desc_mentions_entity' });
            }
        }
    }
