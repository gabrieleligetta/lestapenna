...
const ALL_PHASES: SessionPhase[] = [
    'RECORDING', 'PENDING_PAYMENT', 'TRANSCRIBING', 'CORRECTING', 'SUMMARIZING',
    'INGESTING', 'VALIDATING', 'SYNCING', 'PUBLISHING', 'DONE', 'ERROR'
];

class SessionPhaseManagerImpl {
    private hasIncompleteTranscripts(sessionId: string): boolean {
    ...
    getRecoveryStartPhase(sessionId: string, currentPhase: SessionPhase): 'TRANSCRIBING' | 'SUMMARIZING' | null {
        ...
        // this is needed to recover sessions stuck on a Gemini/OpenAI error after
        // Whisper has already finished.
        if (currentPhase === 'ERROR') {
            return this.hasIncompleteTranscripts(sessionId) ? 'TRANSCRIBING' : 'SUMMARIZING';
        }
